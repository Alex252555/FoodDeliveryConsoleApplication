    ...
    public void run() {
        try {
            ...
            while (true){
                ...
                switch (command){
                    case 1: //Print menu
                        String dishFilter=scan.nextLine();
                        System.out.println(dishFilter);
                        printout.println(Server.menu.size());

                        for(int i=0; i<Server.menu.size(); i++){
                            if(dishFilter.equals("null")){
                                System.out.println("Printing all dishes");
                                ...
                                if(Server.menu.get(i).getFilter().equals(dishFilter)){
                                    printout.println(new String(Integer.toString(i+1)+": "+Server.menu.get(i).getName()+", Filter: ")+Server.menu.get(i).getFilter()+", Description: "+Server.menu.get(i).getDescription()+", Restaurant name: "+Server.menu.get(i).getRestaurant().getName()+", Restaurant address: "+Server.menu.get(i).getRestaurant().getAddress()+", Price: "+Double.toString(Server.menu.get(i).getPrice()));
                                }
                            }
                    ...
                    case 13: //Update restaurant budget
                        String estInfo=scan.nextLine();
                        String[] estParts=estInfo.split("\\|");
                        String establishmentName=estParts[0];
                        String establishmentBudget=estParts[1];

                        for(int i=0;i<Server.registeredEstablishments.size(); i++){
                            if(Server.registeredEstablishments.get(i).getName().equals(establishmentName)){
                                System.out.println(Server.registeredEstablishments.get(i).getBudget());
                                double newBudget=Double.parseDouble(establishmentBudget)+Server.registeredEstablishments.get(i).getBudget();
                                Server.d.updateEstablishmentBudget(Server.c,newBudget,establishmentName);
                            }
                        }
                        Server.d.loadTables(Server.c);
                        break;

                    case 14: //Print establishment budgets
                        for(int i=0; i<Server.registeredEstablishments.size(); i++){
                            System.out.println("here");
                            printout.println("Number: "+Integer.toString(i+1)+", Name: "+Server.registeredEstablishments.get(i).getName()+", Budget: "+Double.toString(Server.registeredEstablishments.get(i).getBudget()));
                        }
                        printout.println("");
                        break;
